...
: CRFFNAME.
  ...
  SETXY FILEINFO FI.FileName UZTYPE ;
...
: NC_SETNAME  ( n -- )
  NC_CUR = 
  DUP	IF FILEINFO FI.FileName COUNT_NAME Z$!
	   FILEINFO FI.Attribute @ EFI_FILE_DIRECTORY  AND TO ?COUNT_DIR
	THEN
 NCSETCOLOR AND  NC_COLOR  XOR 
 FILEINFO FI.Attribute @ EFI_FILE_DIRECTORY  AND
  IF 0x4 XOR
...
\ CCOUT{  ." DIR_DRAW"  CCKWATE }CCOUT
 ...
 NC_CUR    0 MAX TO NC_CUR
  ...
  S" \"  R/O  OPEN-FILE DUP IF S" 0\" $CD ." CD" KEY DROP THEN
  THROW TO DIR_ID
...
: DO_NC_0KEY2_ ( c -- )
  CASE
	KEY2_RIGHT OF NC_CUR  NC_ROWS + TO NC_CUR   ENDOF  \
	KEY2_LEFT OF NC_CUR  NC_ROWS - TO NC_CUR   ENDOF  \ left
	KEY2_DOWN OF NC_CUR  1+ TO NC_CUR
		    NC_CUR  NC_BASE -  NC_COLS NC_ROWS * =
			IF NC_BASE 1+ TO NC_BASE THEN
		 ENDOF
	KEY2_UP OF NC_CUR  NC_BASE - 0=
			IF NC_BASE 1- 0 MAX  TO NC_BASE THEN
			NC_CUR  1- TO NC_CUR
		 ENDOF
	KEY2_HOME OF  NC_HOME ENDOF \ HOME
	KEY2_END OF	 ENDOF \ end
\+ CURSOR%	'R' OF CURSOR% @ 5 U< VSHRIFT@ AND 3 OR CURSOR% !  ENDOF \ Ins

	KEY2_PAGE_UP OF NC_BASE NC_COLS NC_ROWS * - 0 MAX TO NC_BASE
		NC_CUR NC_COLS NC_ROWS * - 0 MAX TO NC_CUR
	     ENDOF \ PAGE UP
	KEY2_PAGE_DOWN OF NC_BASE NC_COLS NC_ROWS * + TO NC_BASE
		NC_CUR	NC_COLS NC_ROWS * + TO NC_CUR
	     ENDOF \ PAGE DOWN
\+ $VIEW	KEY2_F3 OF >R 2>R COUNT_NAME COUNT $VIEW	2R> R> FMTABDO FMTABDO ENDOF \ F3
\+ $EDIT	KEY2_F4 OF >R 2>R COUNT_NAME COUNT $EDIT	2R> R> FMTABDO FMTABDO ENDOF \ F4
 ENDCASE
;
...
: NCDO1B
  CASE
	KEY2_F1 OF ( S"  0 TO NCACCEPT?" ACC_TYPE) ENDOF \ F10
	KEY2_F5 OF
		?COUNT_DIR
		IF	S"  DIRCP " ACC_TYPE
			COUNT_NAME COUNT ACC_TYPE
		...
		THEN ENDOF \ F5
	KEY2_F6 OF  S"  RENAME" GEN_1PARAM S"  NEW"  ACC_TYPE  ENDOF \ F6
	KEY2_F7 OF  S"  MKDIR NEWDIR" ACC_TYPE ENDOF \ F7
	KEY2_F8 OF  S"  RM" GEN_1PARAM  ENDOF \ F8
	KEY2_F9 OF  S"  EFICALL" GEN_1PARAM  ENDOF \ F8
